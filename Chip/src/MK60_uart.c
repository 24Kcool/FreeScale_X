...
*  @note       UART所用的管脚在 App\Inc\PORT_cfg.h 里进行配置，
printf所用的管脚和波特率在 App\Inc\MK60_conf.h 里进行配置
*  Sample usage:       uart_init (UART3, 9600, 1,0);        //初始化串口3，波特率为9600
*/
void uart_init_old(UARTn_e uratn, uint32 baud, uint8 r_watermark, uint8 t_watermark)
{
	register uint16 sbr, brfa;
	uint8 temp;
	uint32 sysclk;     //时钟
	...
	if ((uratn == UART0) || (uratn == UART1))
	{
		//ASSERT(r_watermark > 0 && r_watermark < 9);
		UART_RWFIFO_REG(UARTN[uratn]) = UART_RWFIFO_RXWATER(r_watermark);                                //内核时钟
		//ASSERT(t_watermark >= 0 && t_watermatk < 8);
		UART_TWFIFO_REG(UARTN[uratn]) = UART_TWFIFO_TXWATER(t_watermark);
	}
	/* 允许发送和接收 */
	UART_C2_REG(UARTN[uratn]) |= (0
		| UART_C2_TE_MASK                     //发送使能
